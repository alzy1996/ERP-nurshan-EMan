...
function magicOK(bytes: Uint8Array, sigs: number[][]) {
  return sigs.some((sig) => sig.every((b, i) => bytes[i] === b));
}

/** Cloudinary unsigned upload → secure_url. */
async function upload(blob: Blob, folder?: string): Promise<{ url: string; path: string }> {
...
/** Upload a generated blob (trusted, no magic check). */
export function uploadBlob(blob: Blob, folder?: string) {
